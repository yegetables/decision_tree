...
# 读取原始数据
def lode_origin_data(isPart,cache=False):
    cache_train_file = current_dir+'/data/train_process.csv'
    cache_test_file = current_dir+'/data/test_process.csv'
    if cache and os.path.exists(cache_train_file) and os.path.exists(cache_test_file):
        train_file=cache_train_file
        test_file=cache_test_file
    else:
        if isPart and os.path.exists(current_dir+'/data/train_part.csv') and os.path.exists(current_dir+'/data/testA_part.csv'):
            train_file=current_dir+'/data/train_part.csv'
            test_file=current_dir+'/data/testA_part.csv'
        else:
            train_file=current_dir+'/data/train.csv'
            test_file=current_dir+'/data/testA.csv'
    df_train = pd.read_csv(train_file)
    df_test = pd.read_csv(test_file)
    print(f"load data,cache={cache},isPart={isPart}")
    print("read ",train_file, df_train.shape)
    print("read ",test_file, df_test.shape)
    return df_train,df_test

def analysize_data(df_train,df_test,df):
    # 非数值型
    non_numeric_cols = [
...
# 处理数据
def clear_data(df_train,df_test):
    ...
    test.to_csv(current_dir+'/data/test_process.csv',index=False)
    return train,test
